...
*   get supply chain unit
  ...
  METHOD lif_isolated_doc~sapapo_product_dm_read.
    ...
    IF lv_rec_count > lv_package_size.
      ...
      CLEAR ls_matid.
      CLEAR lt_matid1[].
      LOOP AT it_matid INTO ls_matid ##ENH_OK.
...
*     mapping matid - matnr
        ...
        MOVE-CORRESPONDING <fs_prodlwh_tmp> TO ls_material_lgnum ##ENH_OK.
...
*       Saving user and date/time values of the warehouse product  "Note 2799873
        MOVE-CORRESPONDING ls_material_lgnum TO ls_lgnum_prod ##ENH_OK.
...
** in S4 HANA, some special fields of the warehouse product must be retrieved from MARC
      IF lo_service->is_s4h_stack( ) = abap_true.
        TRY.
          ...
          CATCH /scwm/cx_stockid_map. "#EC NO_HANDLER
        ENDTRY.
...
*  mapping Matnr - Matguid
  ...
  METHOD lif_isolated_doc~get_prod_id.
    DATA:
      ...
      lx_no_material      TYPE REF TO lcx_no_material,    "#EC NEEDED
      lx_not_qualified    TYPE REF TO lcx_not_qualified, "#EC NEEDED
      lv_val              TYPE string,
      lv_line             TYPE string.
...
*       selection of all products from the database in the matnr range
        TRY.
          ...
          CATCH lcx_no_material INTO lx_no_material."#EC NO_HANDLER
          CATCH lcx_not_qualified INTO lx_not_qualified. "#EC NO_HANDLER
        ENDTRY.
