*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZMV_MAP_HMCO
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZMV_MAP_HMCO        .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZMV_MAP_HMCO'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZMV_MAP_HMCO_EXTRACT
TOTAL = ZMV_MAP_HMCO_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZMV_MAP_HMCO'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZMV_MAP_HMCO-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZMV_MAP_HMCO_EXTRACT
TOTAL = ZMV_MAP_HMCO_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZMV_MAP_HMCO-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZMV_MAP_HMCO'
...
TOTAL = ZMV_MAP_HMCO_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZMV_MAP_HMCO-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZMV_MAP_HMCO-UPD_FLAG NE SPACE.
